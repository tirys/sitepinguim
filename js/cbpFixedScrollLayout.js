...
var cbpFixedScrollLayout = (function() {
	...
	var config = {
		// the bg-fixo's sections
		$sections : $( '#bg-fixo > section' ),
		// the navigation links
		$navlinks : $( '#bg-fixo > nav:first > a' ),
	...
	function changeNav( $section ) {
		config.$navlinks.eq( config.currentLink ).removeClass( 'ativo' );
		config.$sections.eq( config.currentLink ).removeClass( 'ativo' );
		config.currentLink = $section.index( 'section' );
		config.$navlinks.eq( config.currentLink ).addClass( 'ativo' );
		config.$sections.eq( config.currentLink ).addClass( 'ativo' );
		if (config.$sections.eq(config.currentLink).hasClass('fundo-branco') && !$('#toggle-nav').is(':checked')) {
