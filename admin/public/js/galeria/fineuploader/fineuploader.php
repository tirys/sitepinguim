...
class qqUploadedFileXhr {
    ...
    public function getSize() {
        if (isset($_SERVER["CONTENT_LENGTH"])){
            ...
            throw new Exception('tamanho do conteudo nao suportado.');
        }      
    }   
}
...
class qqFileUploader {
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE, $filename=''){
        if (!is_writable($uploadDirectory)){
            return array('error' => "Erro de Servidor. Diretorio de upload nao modificavel.");
        }
        
        if (!$this->file){
            return array('error' => 'Nenhum arquivo foi upado.');
        }
        ...
        if ($size > $this->sizeLimit) {
            return array('error' => 'Arquivo muito grande');
        }
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), $this->allowedExtensions)){
            ...
            return array('error' => 'Extensao invalida, deve ser uma dessas '. $these . '.');
        }
